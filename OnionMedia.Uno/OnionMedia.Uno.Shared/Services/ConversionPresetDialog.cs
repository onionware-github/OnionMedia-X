...
namespace OnionMedia.Services
{
    sealed class ConversionPresetDialog : IConversionPresetDialog
    {
        ...
        {
            Uno.Views.Dialogs.ConversionPresetDialog dlg = new(forbiddenNames) { XamlRoot = UIResources.XamlRoot };
        ...
        {
            Uno.Views.Dialogs.ConversionPresetDialog dlg = new(preset, forbiddenNames) { XamlRoot = UIResources.XamlRoot };
            return (await dlg.ShowAsync() is ContentDialogResult.Primary) ? dlg.ConversionPreset : null;
        }
    }
}
